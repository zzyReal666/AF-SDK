...
public class SM1Impl implements SM1 {
     ...
     */
    @Override
    public byte[] SM1Encrypt(GroupMode mode, int index, byte[] key, byte[] data, byte[] IV) throws AFCryptoException {
        return new byte[0];
    }
     ...
     */
    @Override
    public byte[] SM1Decrypt(GroupMode mode, int index, byte[] key, byte[] data, byte[] IV) throws AFCryptoException {
        return new byte[0];
    }
}
